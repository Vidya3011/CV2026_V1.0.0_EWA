...
public class LargeFilesClass extends BaseClass {
	...
	public void Upload_and_verify_Large_PDF_file() throws Exception {
		...
		log("Scenario 01: Add large PDF file");
		// jsclick(pojo.getRefresh_button());
		Thread.sleep(4000);
		jsclick(NewDocuments_MenuOption);
		log("Click on new document menu tab");
		Thread.sleep(4000);
		...
		log("Click on select folder dialog OK button");
		Thread.sleep(4000);
		...
		log("Click on save tab OK button");
		Thread.sleep(10000);
		log("Large PDF file added successfully...");
		jsclick(Refresh_Button(driver));
		// log("User click on refresh icon");
	}

	public void Upload_and_verify_Large_TIFF_file() throws Exception {

		// jsclick(pojo.getRefresh_button());
		log("Scenario 02: Add large TIFF file");
		try {
			loginCVS();
		} catch (Exception e) {
		...
		jsclick(Select_FolderDest);
		Thread.sleep(4000);
		log("Click on browse folder dialog OK button");
		log("Folder is selected for Create New Documents");
		jsclick(OK_Button_BrowseforFolder);
		Thread.sleep(4000);
		log("Select document type as CVReports");
		getSelect_Document_Type_Dropdown();
		Thread.sleep(4000);
		...
		Thread.sleep(10000);
		log("Large Tiff file added successfully...");
	...
	public void Upload_and_verify_Large_DOCX_file() throws Exception {
		...
		Thread.sleep(4000);
		log("Scenario 03: Add Large Word(DOCX) file");
		try {
			loginCVS();
		} catch (Exception e) {
			System.out.println("Browser Not Closed");
		...
		Thread.sleep(5000);
		log("Add large word file by using AutoIT script");
		try {
			WebDriverWait wait1 = new WebDriverWait(driver, Duration.ofSeconds(10));
			wait1.until(ExpectedConditions.alertIsPresent());
			Alert alt = driver.switchTo().alert();
		...
		log("Click on save dialog OK button");
		Thread.sleep(10000);
		log("Large Docx file added successfully...");

		jsclick(Refresh_Button(driver));
	}

	public void Upload_and_verify_Large_XLSX_file() throws Exception {
		log("Scenario 04: Add Large EXCEL file");
		...
		Thread.sleep(4000);
		jsclick(Destination_Folder_Textbox);
		log("Select folder from destination folder dialog");
		Thread.sleep(4000);
		ElementToBeClickable(Select_CabinetDest);
		log("Expand cabinet");
		selectElement(Select_CabinetDest);
		...
		log("Click on save button");
		Thread.sleep(10000);
		log("Click on save dialog button");
		log("Large xlxs file added successfully...");
