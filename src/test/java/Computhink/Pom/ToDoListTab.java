...
public class ToDoListTab extends BaseClass {
	...
	public void AnnotationPageSendWF() throws Exception {
		...
		log("Scenario 01:Workflow with Annotations added check");
		Thread.sleep(4000);
		...
		log("Select Dynamic workflow document type");
		Thread.sleep(4000);

		scrollDown(DynamicDocumentWF);
		log("Click on User1 textbox");
		DynamicDocumentWF.click();
		Thread.sleep(3000);
		log("Select Task user1");
		log("Click on Select task user dialog OK button");
		...
		jsclick(findbtn);
		Thread.sleep(3000);
		log("Click on User2 text box");
		jsclick(RNishaUser);
		log("Select Task user2");
		log("Click on Select task user dialog OK button");
		movingclkElement(usergrpOKBtn);
		Thread.sleep(4000);
		jsclick(User2);
		log("Click on User_3 textbox");
		Thread.sleep(3000);
		log("Select Task user3");
		log("Click on Select task user dialog OK button");
		jsclick(findbtn);
		jsclick(User1Vidya);
		movingclkElement(usergrpOKBtn);
		Thread.sleep(4000);
		log("USer click on Select user dialog OK button");
		jsclick(User3);
		...
		log("Enter data in key value");
		KeyValue.sendKeys("AllUserAccept");

		Thread.sleep(3000);
		log("Mousehover on add icon");

		movingElement(moveToPlusIcon);
		Thread.sleep(5000);
		log("Select a browse option");
		Thread.sleep(5000);
		VisiblityOf(browseOption);
		movingclkElement(browseOption);
		log("Add a file by using AutoIT script");
		Thread.sleep(5000);
	...
	public void LoggedInAsThirdUser() throws Exception {
		...
		log("TaskUser3 Dipak Logged in successful");
	}

	public void VerifyingAnnotationPageByThirdUSerDipak() throws Exception {

		VisiblityOf(TodolistTab);
		log("Select todolist tab");
		jsclick(TodolistTab);
		log("Select newitems");
		Thread.sleep(3000);
		jsclick(NewItems);
		Thread.sleep(5000);
		log("Open first document");
		jsclick(Open1stDocument);
		Thread.sleep(3000);
		try {
			alertIsPresent();
			...
			log("Alert not presert");
		}
		Thread.sleep(4000);
		log("User click on accept button");
		AcceptButton.click();
		log("Alert Message is Displayed");
		Thread.sleep(8000);
		...
		Thread.sleep(2000);
		log("Select update property");
		jsclick(Update4thUser);
		log("CLick user_4 textbox");
		Thread.sleep(2000);
		jsclick(findBTN);
		log("User select Ram named user");
		Thread.sleep(2000);
		jsclick(RamUSer);
		log("Click on user dialog OK button");
		Thread.sleep(3000);
		jsclick(OKBTNADDName);
		Thread.sleep(3000);
		log("Click on properties update save button");
		jsclick(PropertiesUpdateSaveBTN);
		VisiblityOf(UpdateDialogOKBTN);
		jsclick(UpdateDialogOKBTN);
		Thread.sleep(5000);
		log("Now the user can able to sent the document to workflow ");
		jsclick(AcceptButton);
		Thread.sleep(3000);
		log("User click on accept button");
		WorkflowComments.sendKeys(TodoListExcel(5, 1));
		log("USer enter a comment and click on comment dilaog OK button");
		jsclick(WorkflowCommentsOKButton);
		Thread.sleep(8000);
	}
	...
	public void VerifyingAnnotationPageByFourthUSerRAm() throws Exception {
		...
		Thread.sleep(3000);
		jsclick(NewItems);
		log("Select a newitems");
		Thread.sleep(5000);
		log("Open a first wf document");
		jsclick(Open1stDocument);
		Thread.sleep(3000);
		try {
			alertIsPresent();
		...
		Thread.sleep(7000);
		log("Click on accept button");
		AcceptButton.click();
		log("The document accepted by 4th user.");
		WorkflowComments.sendKeys(TodoListExcel(6, 1));
		jsclick(WorkflowCommentsOKButton);
		log("Workflow with all user accpet condition Scenario working fine");
		Thread.sleep(8000);
	}
	...
	public void SentItemsWithSummary() throws InterruptedException {
		...
		log("Sent items (MetaData) functionality with  summary workflow status validation successfull");
	...
	public void LogRnishaTaskUser1() throws Exception {
		Thread.sleep(4000);
		LogoutPage();
		log("Scenario 02: Task User Log into EWA");

		Thread.sleep(3000);
		WebElement Username = driver.findElement(By.xpath("//input[@id='userName']"));
		Thread.sleep(3000);
		log("Enter task user name into user name field ");
		...
		try {
			...
			log("Login without session message...");
		}

		log("Task user logged into EWA successfully...");
	}
	...
	public void SendingDocumentToworkflow() throws Exception {
		...
		log("Scenario 01: Document send to manual workflow");
		Thread.sleep(3000);
		...
		CheckBox.click();
		log("Select any document from the document list");
		
		movingElement(DocumentList);
		log("Mousehover on document tab");*/
		...
		jsclick(SendToWF);
		Thread.sleep(3000);
		log("Select sendto workflow option");

		jsclick(AnyWorkflow);
		log("Select any workflow");
		Thread.sleep(2000);
		...
		log("Document send to workflow successfully dialog will be appeared...");
		Thread.sleep(3000);
		log("Click confirmation dialog OK button");

		log("Document send to manual workflow successfully");
	...
	public void refrshLog3rdTaskUserNishaC() throws Exception {
		LogoutPage();
		log("Scenario 06: Task user log into EWA");
		Thread.sleep(3000);
		log("Enter valid task user name into user name field");
		...
		log("Task user log into EWA successfully...");
	}
	...
	public void LogVidyaTaskUser1() throws Exception {
		...
		LogoutPage();
		log("Scenario 4: Task user Log into EWA ");
		Thread.sleep(3000);
		...
		log("Enter valid task name into user name field");
	...
	public void RamTaskUser() throws Exception {
		...
		try {
			...
			log("Login without msg");
		}
		log("User login as Ram user");
	}
	...
	public void DynamicWorkFlowTodoListFunctionality() throws Exception {
		...
		Thread.sleep(4000);
		log("Scenario 01: Document send to - Dynamic workflow");
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
		VisiblityOf(newDocumentTab);
		...
		jsclick(sendToWf);
		log("Click SendtoWorkflow button");
		Thread.sleep(3000);

		jsclick(dynamicwfnew);
		log("Select any dynamic workflow");
		...
		jsclick(sendwfsuccessdialogOKBTN);
		log("Document sent to workflow Successfully dialog will appear...");
		log("Click confirmation dialog OK button");
	...
	public void dynamicWF_MetaDataAgree_Vidya() throws Exception {
		...
		log("Scenario 03: Task user1 Accept the dynamic Workflow document");
		...
		// Select New items
		log("Select New items");
		jsclick(NewItems);
		Thread.sleep(5000);

		// Expand workflow document metadata
		movingclkElement(Metadata);
		log("Expand workflow document metadata");
		Thread.sleep(3000);

		// Click on comment button and add comment
		CommentButton.click();
		log("Click on comment button");
		CommenttextBox.sendKeys(TodoListExcel(4, 0));
		Thread.sleep(3000);
		log("Add comment into comment dialog");
		jsclick(CommenttextBoxOKBTN);
		log("Click on comment dialog OK button");
		Thread.sleep(4000);

		// Click on Accept button
		log("click on Accept button");
		AcceptButton.click();
		log("Enter comment into the Accept dialog box");
		...
		jsclick(WorkflowCommentsOKButton);
		log("Click on Accept dialog OK button");
		Thread.sleep(8000);
		...
		log("TaskUser1 Accept the document successfully...");
	...
	public void DynamicWF_user2_RNishaAcceptTheDocument() throws Exception {
		...
		log("Scenario 05: Task user2 Accept dynamic Workflow document ");
		jsclick(TodolistTab);
		log("Select Todolist tab");
		Thread.sleep(3000);
		...
		jsclick(TodolistDoc);
		log("Open a received workflow document");
		Thread.sleep(6000);
		...
		log("Enter comment and click on comment dialog OK button");
		CommenttextBox.sendKeys(TodoListExcel(4, 0));
		Thread.sleep(3000);
		jsclick(CommenttextBoxOKBTN);
		...
		log("Enter comment into Accept dialog box");
		...
		log("Task user2 accepted the document successfully...");
		log("Task user2 logout the session successfully...");

		
	}

	public void DynamicWF_user3_DipakRejectTheDocument() throws Exception {
		log("Scenario 07: Task User 3 Reject the dynamic workflow document");
		SoftAssert so = new SoftAssert();  // Create SoftAssert instance

		log("Select a todolist tab");
		jsclick(TodolistTab);
		Thread.sleep(3000);
		...
		log("Expand any document metadata option");
		...
		log("Select Reject button");
		jsclick(RejectButton);
		Thread.sleep(3000);

		log("Enter comment in the Reject dialog box");
		...
		jsclick(Refresh_Button(driver));
		log("Click on Reject dialog box OK button");
		log("Task user_3 Rejected the document");
	...
	public void Originator_Pending_Items() throws Exception {
		...
		log("Scenario 09: Originator reviewed the  dynamic workflow document");
		...
		movingclkElement(Metadata);
		Thread.sleep(3000);
		log("Select Review button");
		jsclick(ReviewButton);
		Thread.sleep(3000);
		log("Enter comment in the Review dialog box");
		WorkflowComments.sendKeys(TodoListExcel(1, 5));
		Thread.sleep(3000);
		WorkflowCommentsOKButton.click();
		...
		Thread.sleep(8000);
		jsclick(Refresh_Button(driver));
		log("Document Reviewed Successfully");
		log("Originator Reviewed the document");
	...
	public void SendAndNewItemsMetaDataAgree() throws Exception {
		log("Scenario 03:Task User1 Accept the workflow document");
		
		SoftAssert so = new SoftAssert();
		Thread.sleep(3000);
		movingElement(TodolistTab);
		log("Click on Todolist tab");
		Thread.sleep(3000);

		jsclick(TodolistPendingItems);
		log("Select Newitems option");
		Thread.sleep(5000);
		ElementToBeClickable(Metadata);
		movingclkElement(Metadata);
		Thread.sleep(3000);
		log("Expand a document metadata option");
		AcceptButton.click();
		log("Click on Accept button");
		...
		log("Add comment in Accept dialog");
		jsclick(WorkflowCommentsOKButton);
		log("Click on Accept dialog OK button");
		Thread.sleep(8000);

		log("Task user1 Accept the document successfully...");

		log("Task user1 Logout the Session...");
	}
	...
	public void TaskUser2RNishaRejectTheDocument() throws Exception {
		...
		log("Scenario 05: Task user2 Accept the workflow document");
		...
		ElementToBeClickable(Metadata);
		movingclkElement(Metadata);
		log("Expand workflow document metadata option");
		Thread.sleep(3000);
		...
		log("Add comment in Reject dialog box");
		...
		log("Click on Reject dialog OK button");
		log("Rejected Document Move to Task User3 successfully...");
		log("Click on Reject dialog OK button");
		log("Task user 2 logout the session successfully...");
	...
	public void SummaryStatusDialogBoxFucntions() throws InterruptedException {
		...
		jsclick(TodolistTab);
		log("USer click on todolist tab");
		Thread.sleep(3000);
		jsclick(NewItems);
		log("Select new items submenu");
		Thread.sleep(5000);
		VisiblityOf(Metadata);
		log("User Expand the todolist newitems documents's metadata");
		clickElement(Metadata);
		Thread.sleep(5000);
		log("User click on summary button");
		jsclick(SummaryButton);
		Thread.sleep(5000);
		log("USer check the summary of the document");
		jsclick(SummaryButton);
		log("User Expand the todolist newitems documents's metadata");
		log("Task user3 Dipak has shows the Summary of a document it shows added comments successfully");
