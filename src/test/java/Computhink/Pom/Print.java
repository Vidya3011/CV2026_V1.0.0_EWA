...
public class Print extends Computhink.Generic.BaseClass {
	...
	public void Email_Reference() throws Exception {
		...
		WebElement successDialog = driver.findElement(By.id("EmailsuccessMessageDialogbox")); // Use the appropriate
																								// locator here
	...
	public void OpenDocumentAndCheckPages() throws Exception {
		...
		for (int i = 0; i < driver.findElements(By.xpath("//*[@id='documentListTable']/tbody/tr/td[3]")).size(); i++) {
			try {
				...
				List<WebElement> documentNames = driver
						.findElements(By.xpath("//*[@id='documentListTable']/tbody/tr/td[3]"));
				...
				if (pages.isEmpty()) {
					...
					boolean hasContent = pages.stream().anyMatch(page -> {
	...
	public void DocumentSendToSecureLink() throws Exception {
		...
		// correctly
		String expectedEmail = readFromExMail(2, 0);
		softAssert.assertEquals(secureLinkReceipientMail.getAttribute("value"), expectedEmail,
				"Recipient email is incorrect.");
		softAssert.assertEquals(secureLinkConfirmMail.getAttribute("value"), expectedEmail,
	...
	public void SecureLinkFromDocumentList() throws Exception {
		...
		softAssert.assertEquals(secureLinkConfirmMail.getAttribute("value"), expectedEmail,
				"Confirm email is incorrect.");
		softAssert.assertEquals(secLinkReceipientMailAgain.getAttribute("value"), expectedEmail,
				"Recipient email again is incorrect.");
		softAssert.assertEquals(secLinkConfirmtMailAgain.getAttribute("value"), expectedEmail,
				"Confirm email again is incorrect.");
	}
	...
	public void SecureLinkFromSearchTab() throws Exception {
		...
		// SoftAssert: Verify if the email and confirm email fields match the expected
		// values
		String expectedEmail = readFromExMail(1, 0);
		softAssert.assertEquals(secureLinkReceipientMail.getAttribute("value"), expectedEmail,
				"Recipient email is incorrect.");
